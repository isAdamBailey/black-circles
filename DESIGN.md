...
components:
  ...
  button-accent:
    ...
    rounded: "{rounded.lg}"
    padding: "12px 20px"
    note: "Reserved for the mood-search 'Find it' button and the Random release action beside it — the app's two accent-bearing surfaces."
  button-accent-hover:
...
**The home-page exception.** "Find it" and "Random release" — the two ways to get a record right now, sitting side by side in the same row — are the one deliberate departure from the Rarity Rule, sharing the Oxblood/Oxblood Bright treatment as the page's signature color. "Browse collection" is not part of this exception; it's a quiet Shelf/secondary link, since browsing isn't a decision the page is making for you. Every other page keeps strict rarity: Secondary buttons elsewhere stay Shelf/Groove, and Oxblood does not spread to the collection grid, auth pages, or nav — including the Random release button repeated there, which stays Shelf/secondary.
...
- **Subhead** (700, 1.5rem / 24px, lh 1.2, ls -0.01em): The one tier between Display and Headline, reserved for a page's single promoted primary interaction that needs more weight than a Headline but isn't the page title — e.g. "Find something to play" on the home page. Not for general section headings; use Headline for those.
...
Focus rings use the Groove border color (`#374151`) by default; the "Find it" button, its mood-search input, and the "Random release" button beside it (the app's accent-bearing surfaces) use Oxblood Bright (`#c23a42`) instead, since the base Oxblood fails 3:1 contrast against Void. Ring style: `outline: 2px solid; outline-offset: 2px`. No glow.
...
- **Accent:** Oxblood surface (`#7c1f25`) with Pressing text, hover shifts to Oxblood Bright (`#c23a42`). Deliberately scoped to the home page's "Find it" and "Random release" pair, sitting together in the mood-search row, where it reads as the page's own signature color rather than a single one-off signal. This is an intentional, page-scoped exception to the rarity rule below; it does not extend to "Browse collection" on the same page, or to Secondary buttons on other pages (collection grid, auth, profile) — including the Random release button repeated on the collection page and the "Shuffle again" action on release detail pages, which all stay on the Shelf/Groove treatment.
...
- **Don't** let the oxblood accent spread beyond the home page's "Find it" / "Random release" pair and the mood-search input's focus state — that pair is a deliberate, documented exception to rarity, not a precedent. "Browse collection" and every other button on every other page (including the repeated Random release and Shuffle again actions) stay Shelf/Groove. Don't reuse Oxblood for other CTAs, badges, or large text on other pages.
